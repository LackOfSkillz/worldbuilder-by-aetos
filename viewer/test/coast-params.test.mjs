...
test("the joint bounds are the engine's, asked of the engine", () => {
  ...
  assert.throws(
    ...
    // loosening. `newWorld` now calls the widest door, `wb_world_new_water` -- the carve
    // channel's wiring, past `wb_world_new_peak` -- so the message names that export. What this assertion
    // is about is unchanged and is the second half: a refused world says WHICH channel refused
    // it, rather than handing back a handle of 0 and a blank page.
    /wb_world_new_water refused.*coast=WB_ERR_PARAM/s,
  );
});
