...
| R-3a | Where no candidate at a station is above the datum, the tracer steps back toward its chord in half-spacing increments and takes the first lateral above it, the chord point last; if even the chord point is at or below the datum, the chord point is kept. The one exception to R-3. | Holding the line instead left 158 inland stations of 440,985 on sea ground, 108 of them with the chord point on land, up to 100 km sideways on a coast (final review, Ruling FF-2). | A chord across a bay keeps its stations on the water it crosses, which stage 2 carves as it finds it. |
