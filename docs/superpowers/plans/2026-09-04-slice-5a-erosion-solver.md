...
## CORRECTED: what m = 0.5 and n = 1 actually buy (and what they do not)

**An earlier draft of this plan claimed this slice could hold the STRICT bit-for-bit contract because the
exponents avoid transcendentals. That claim was wrong on both halves, and the correction matters.**

**Wrong half one: the contract split does not apply here at all.** Strict-versus-bounded governs
*Python-against-Rust* conformance, and it exists because CPython uses the platform libm while the engine uses
the pure-Rust `libm` crate. **Erosion has no Python counterpart** — `worldbuilder/` has no stream power, and
this plan forbids writing one. So the only equality claims here are *native against WASM* and *run against
run*, and **native and WASM both use the same pure-Rust `libm`, so they agree bit-for-bit even through
transcendentals.** There is no tolerance to spend and no contract to upgrade.

**Wrong half two: the solver's path contains a transcendental regardless.** Slope needs the distance from a
node to its receiver, and `SpherePoint::distance_to` is `angle_to * radius_m` where `angle_to` is
`m::atan2(across, along)` — verified in `src/sphere.rs:58-66`. Positions are not stored on the graph at all
(`GraphHeader` keeps only a `position_checksum`); they are regenerated by
`stream::node_positions(world_seed, count)`. **Any slope computation goes through `atan2`.**

**What the exponents genuinely buy**, and it is still worth having:

- `A^0.5` is `sqrt`, which IEEE-754 requires to be correctly rounded, so it is exact and cheap.
- `s^1` is `s`, so **`detmath::powf` never appears in the inner loop** — which matters for cost, since this
  is a 100-300 iteration bake over up to 20 M nodes, and `powf` is far more expensive than a multiply.

`hypot` is *not* `sqrt` and must not be substituted for it: CPython has computed its own Neumaier-summed norm
since 3.8, so they are different algorithms.

**If a task finds itself reaching for `powf` in the inner loop, stop and say so** — it means the formulation
drifted from the spec's exponents. `atan2` in distance computation is expected and correct.
