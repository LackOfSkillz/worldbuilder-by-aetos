...
Six gates run: the engine suite, a determinism guard, the Python conformance suite (which now
rejects both a missing *and* a stale `worldbuilder_engine`), a fingerprint-parity check
between two independent implementations of the same digest algorithm, `.wasm` provenance, and
native/wasm parity - plus two count gates that catch a suite quietly shrinking while every
exit-status check stays green. All six exist because specific, once-real failures survived
multiple commits: a conformance suite that skipped silently and reported green while
comparing nothing; a shipped `.wasm` that was several commits stale and still passed parity;
a provenance guard that only worked on its author's machine; and two Python distributions
that shared one name, so the reference package was never actually installed while the suite
passed anyway on a `pytest` path accident. The full account - every gate's exact failure
message, the two-digest ruling that permits the fingerprint's Rust reimplementation, how the
stale-engine guard's soundness depends on a second gate having already run, and what still
has no CI coverage - is in **[`docs/ci.md`](docs/ci.md)**, re-derived from the current tree
rather than quoted from an earlier report.

## Layout

    CHANGELOG.md                             phase by phase, with the measurements
    docs/ci.md                               every CI gate, its exact failure message, and what still isn't covered
